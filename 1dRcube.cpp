...
class array1dRcube : public GenericRcube {
private:
...
public:
    ...
    bool operator==(const array1dRcube &r1) const {
        for (int i = 0; i < 54; i++) {
    ...
    array1dRcube &operator=(const array1dRcube &r1) {
        for (int i = 0; i < 54; i++) {
            cube[i] = r1.cube[i];
        }
        return *this;
...
struct Hash1d {
    size_t operator()(const array1dRcube &r1) const {
        string str = "";
